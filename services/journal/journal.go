...
//		return nil, nil
//	}
func GetPastFile(id string, time time.Time) (*file_model.WeblensFileImpl, error) {
	lt := j.Get(id)
	if lt == nil {
		return nil, errors.WithStack(errors.ErrNohistory.FileAction.WithArg(id))
	}
	...
	path := fs.ParsePortable(relevantAction.DestinationPath)

	f := file_model.NewWeblensFile(id, path.Filename(), nil, path.IsDir())
...
func GetActionsByPath(ctx context.Context, path fs.Filepath) ([]*history.FileAction, error) {
	return getActionsByPath(path, false)
}
