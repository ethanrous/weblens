...
if [ -z "$(sudo docker images -q weblens-go-build-"${arch}" 2> /dev/null)" ]; then
    ...
    sudo docker build -t weblens-go-build-"${arch}" --build-arg ARCHITECTURE="$arch" -f GoBuild .
fi
...
sudo docker run -v ./api/src:/source -v ~/weblens-build-cache/go-pkg:/go -v ~/weblens-build-cache/go-build:/root/.cache/go-build --platform "linux/$arch" --rm weblens-go-build-"${arch}" /bin/bash -c \
"cd /source && export GIN_MODE=release && CGO_ENABLED=1 GOOS=linux GOARCH=$arch go build -v -ldflags=\"-s -w\" -o weblens"

sudo docker build --platform "linux/$arch" -t ethrous/weblens:"${docker_tag}-${arch}" --build-arg build_tag="$docker_tag" .
