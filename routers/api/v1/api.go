...
func Routes(_ context_service.AppContext) *router.Router {
	...
	r.Group("/files", func() {
		r.Group("/{fileID}", func() {
			r.Get("", router.RequireFilePermissions(share_model.SharePermissionView), file_api.GetFile)
			r.Get("/text", router.RequireFilePermissions(share_model.SharePermissionView), file_api.GetFileText)
			r.Get("/stats", router.RequireFilePermissions(), file_api.GetFileStats)
			r.Get("/download", router.RequireFilePermissions(share_model.SharePermissionDownload), file_api.DownloadFile)
			r.Get("/history", router.RequireFilePermissions(share_model.SharePermissionView), file_api.GetFolderHistory)
		})
	})
	...
	r.Group("/files", func() {
		...
		r.Group("/{fileID}", func() {
			r.Patch("", router.RequireFilePermissions(), file_api.UpdateFile)
		})
	}, router.RequireSignIn)
	...
	r.Group("/folder", func() {
		r.Group("/{folderID}", func() {
			...
			r.Patch("/cover", router.RequireFilePermissions(share_model.SharePermissionEdit), file_api.SetFolderCover)
			r.Delete("/cover", router.RequireFilePermissions(share_model.SharePermissionEdit), file_api.RemoveFolderCover)
		})
