...
func Create(ctx ctxservice.RequestContext) {
	userParams, err := netwrk.ReadRequestBody[wlstructs.NewUserParams](ctx.Req)
	if err != nil {
		return
	}
...
func Login(ctx ctxservice.RequestContext) {
	userCredentials, err := netwrk.ReadRequestBody[wlstructs.LoginParams](ctx.Req)
	if err != nil {
		ctx.Error(http.StatusBadRequest, err)
...
func GetAll(ctx ctxservice.RequestContext) {
	...
	results := make([]wlstructs.UserInfo, 0, len(users))
...
func UpdatePassword(ctx ctxservice.RequestContext) {
	...
	updateParams, err := netwrk.ReadRequestBody[wlstructs.PasswordUpdateParams](ctx.Req)
	if err != nil {
		ctx.Error(http.StatusBadRequest, err)
