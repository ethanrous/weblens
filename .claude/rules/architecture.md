...
**Eligibility**: `media.IsEmbeddable` (a media-type registry flag) drives `EmbedEligible(ext)` — every type the pipeline touches. `TextEmbedEligible(ext)` = embeddable AND not image-recognizable (docs/code/OCR types, but not photos: photos embed visually and OCR on them yields junk).

**Background job**: `job.ExtractAndEmbedTask` (registered in `services/jobs/jobs.go`, handler `ExtractAndEmbedFile` in `services/jobs/embed.go`). Dispatched via the shared `dispatchEmbedTask` helper (`services/jobs/file_parser.go`), wired into every index entry point: `IndexFile` (for displayable media, after its media row + thumbnail cache exist — so the image embed never races ahead of them), `queueFileIndexIfNeeded` (non-displayable docs, before the displayability gate), `uploadCleanup`, the `ScanDir` endpoint, and the websocket scan handler. Always dispatched to an independent pool so directory scans finish fast and embeddings trail. The handler self-gates on the feature flag, `EmbedEligible`, size, and service availability; writes image embeddings for files that have a media row; then, for `TextEmbedEligible` files, calls `/extract-and-embed`, upserts each chunk via `embedding.Upsert`, and prunes trailing rows via `embedding.PruneTrailingChunks`. Idempotent via `CountByContentID` / per-page `CountForChunk`.

**Image embeddings**: written by `ExtractAndEmbedFile` (`services/jobs/embed.go:writeImageEmbedding`) — not inline during scan. `IndexFile` dispatches the embed task only after the media row + thumbnail cache exist, so the image embed can't race them. Iterates `media.PageCount` so multi-page documents get one vector per page (HighRes cache file); single-page media use the LowRes thumbnail. Gated on `SupportsImgRecog`; per-page existence is checked, so partial embeddings backfill on rerun.

**Force re-index** (`forceReindex` query param / "Re-Index Folder" action): `IndexDirectory.clearExistingIndex` deletes media, both embedding kinds (`embedding.DeleteAllForSources`), and cache for the discovered batch; the single-file `ScanFileTsk` force path does the same per file before rebuilding.

**Search merge**: `/files/search` (`routers/api/v1/file/rest_files_search.go:SearchFiles`) runs filename substring/regex + semantic vector search in parallel via `errgroup`. Semantic path encodes the query, calls `embedding.Search`, rescales raw cosines per kind via `embedding.ScoreHits` (a hit survives if it stands out above its kind's mean or sits well above the kind's noise floor → `logistic`, so image and text hits are comparable), demuxes image hits via `media.contentID → []*file`, blends with filename scores. Response: `[]SearchResult{file, matchKind ([]string; may contain "filename" and/or "content"), matchSnippet, matchPage, score}`. Semantic path is non-fatal — it's skipped silently when the embed service is unavailable, when `embed.processing_enabled` is off, when `regex=true`, or when `includeContent=false`.
...
**Feature flag**: `embed.processing_enabled` in `feature_flags`. When false, `ExtractAndEmbedTask` no-ops (the handler self-checks the flag), so neither image nor text embeddings are written; the folder-listing embed-backfill in `formatRespondFolderInfo` also skips. Folder listings only re-dispatch an embed-driven scan when the flag is on and the service is reachable.
