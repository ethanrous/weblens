...
**Search merge**: `/files/search` (`routers/api/v1/file/rest_files.go:SearchFiles`) runs filename fuzzy + semantic vector search in parallel via `errgroup`. Semantic path encodes the query, calls `embedding.Search`, demuxes image hits via `media.contentID → []*file`, blends scores. Response: `[]SearchResult{file, matchKind ("filename"/"content"/"both"), matchSnippet, score}`. Semantic path is non-fatal — it's skipped silently when the embed service is unavailable, when `regex=true`, or when `includeContent=false`.
