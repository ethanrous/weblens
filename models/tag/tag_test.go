...
func TestTag_GetFileIDsByTagIDs(t *testing.T) {
	ctx := db.SetupTestDB(t, tag.TagCollectionKey, tag.IndexModels...)

	tag1, err := tag.CreateTag(ctx, "Red", "#e74c3c", "alice")
	require.NoError(t, err)
	tag2, err := tag.CreateTag(ctx, "Blue", "#3498db", "alice")
	require.NoError(t, err)

	// file-1 in both tags, file-2 only in tag1, file-3 only in tag2
	err = tag.AddFilesToTag(ctx, tag1.TagID, []string{"file-1", "file-2"})
	require.NoError(t, err)
	err = tag.AddFilesToTag(ctx, tag2.TagID, []string{"file-1", "file-3"})
	require.NoError(t, err)

	t.Run("SingleTag", func(t *testing.T) {
		fileIDs, err := tag.GetFileIDsByTagIDs(ctx, []primitive.ObjectID{tag1.TagID})
		require.NoError(t, err)
		assert.Len(t, fileIDs, 2)
		assert.Contains(t, fileIDs, "file-1")
		assert.Contains(t, fileIDs, "file-2")
	})

	t.Run("MultipleTags_Intersection", func(t *testing.T) {
		// AND logic: only file-1 is in both tags
		fileIDs, err := tag.GetFileIDsByTagIDs(ctx, []primitive.ObjectID{tag1.TagID, tag2.TagID})
		require.NoError(t, err)
	...
	t.Run("NoMatchingTags", func(t *testing.T) {
		fileIDs, err := tag.GetFileIDsByTagIDs(ctx, []primitive.ObjectID{primitive.NewObjectID()})
		require.NoError(t, err)
		assert.Empty(t, fileIDs)
	})

	t.Run("EmptyTagIDs", func(t *testing.T) {
		fileIDs, err := tag.GetFileIDsByTagIDs(ctx, []primitive.ObjectID{})
		require.NoError(t, err)
		assert.Empty(t, fileIDs)
	})
}
