...
func getTestCtx(t *testing.T) context.Context {
	ctx := db.SetupTestDB(t, usermodel.UserCollectionKey)
	ctx = context.WithValue(ctx, cryptography.BcryptDifficultyCtxKey, bcrypt.MinCost) // Set bcrypt difficulty for testing
...
func TestUser_Creation(t *testing.T) {
	...
	t.Run("CreateValidUser", func(t *testing.T) {
		...
		err := usermodel.SaveUser(ctx, usr)
	...
	t.Run("CreateDuplicateUser", func(t *testing.T) {
		...
		err := usermodel.SaveUser(ctx, user1)
		assert.NoError(t, err)

		err = usermodel.SaveUser(ctx, user2)
		assert.Error(t, err)
	})
...
func TestUser_Authentication(t *testing.T) {
	...
	t.Run("ValidLogin", func(t *testing.T) {
		...
		retrievedUser, err := usermodel.GetUserByUsername(ctx, usr.Username)
		assert.NoError(t, err)
...
func TestUser_Permissions(t *testing.T) {
	...
	t.Run("PermissionLevels", func(t *testing.T) {
		usr := &usermodel.User{
			...
			Password:    testPassword,
			DisplayName: testDisplayName,
			UserPerms:   usermodel.UserPermissionBasic,
		}
		...
		usr.UserPerms = usermodel.UserPermissionAdmin
		assert.True(t, usr.IsAdmin())
		assert.False(t, usr.IsOwner())
		assert.False(t, usr.IsSystemUser())

		usr.UserPerms = usermodel.UserPermissionOwner
		assert.True(t, usr.IsAdmin())
		assert.True(t, usr.IsOwner())
		assert.False(t, usr.IsSystemUser())

		usr.UserPerms = usermodel.UserPermissionSystem
		assert.True(t, usr.IsAdmin())
		assert.True(t, usr.IsOwner())
	...
	t.Run("PermissionUpdates", func(t *testing.T) {
		...
		err = usr.UpdatePermissionLevel(ctx, usermodel.UserPermissionAdmin)
		assert.NoError(t, err)

		retrievedUser, err := usermodel.GetUserByUsername(ctx, usr.Username)
		assert.NoError(t, err)
		assert.Equal(t, usermodel.UserPermissionAdmin, retrievedUser.UserPerms)
	})
}
...
func TestUser_Retrieval(t *testing.T) {
	...
	t.Run("GetUserByUsername", func(t *testing.T) {
		...
		retrieved, err := usermodel.GetUserByUsername(ctx, usr.Username)
		assert.NoError(t, err)
	...
	t.Run("GetAllUsers", func(t *testing.T) {
		...
		col, err := db.GetCollection[any](ctx, usermodel.UserCollectionKey)
		require.NoError(t, err)
		err = col.Drop(ctx)
		require.NoError(t, err)
	...
	t.Run("GetServerOwner", func(t *testing.T) {
		owner := &usermodel.User{
			...
			UserPerms:   usermodel.UserPermissionOwner,
			Activated:   true,
		}

		err := usermodel.SaveUser(ctx, owner)
		require.NoError(t, err)

		retrievedOwner, err := usermodel.GetServerOwner(ctx)
		assert.NoError(t, err)
		assert.NotNil(t, retrievedOwner)
		assert.Equal(t, owner.Username, retrievedOwner.Username)
		assert.Equal(t, usermodel.UserPermissionOwner, retrievedOwner.UserPerms)
	})

	t.Run("UserNotFound", func(t *testing.T) {
		_, err := usermodel.GetUserByUsername(ctx, "nonexistent")
		assert.Error(t, err)
	})
}
...
func TestUser_Delete(t *testing.T) {
	...
	t.Run("DeleteUser", func(t *testing.T) {
		...
		_, err = usermodel.GetUserByUsername(ctx, usr.Username)
		assert.Error(t, err)
	})
	...
	t.Run("DeleteAllUsers", func(t *testing.T) {
		...
		err := usermodel.DeleteAllUsers(ctx)
		assert.NoError(t, err)
...
func TestUser_Search(t *testing.T) {
	...
	t.Run("SearchByUsername", func(t *testing.T) {
		...
		results, err := usermodel.SearchByUsername(ctx, "john")
		assert.NoError(t, err)
		assert.Len(t, results, 2)

		results, err = usermodel.SearchByUsername(ctx, "doe")
		assert.NoError(t, err)
		assert.Len(t, results, 2)

		// Test exact match
		results, err = usermodel.SearchByUsername(ctx, "john_doe")
		assert.NoError(t, err)
		assert.Len(t, results, 1)

		// Test no matches
		results, err = usermodel.SearchByUsername(ctx, "nonexistent")
		assert.NoError(t, err)
