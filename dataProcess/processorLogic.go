...
func scanFile(t *task) {
	...
	meta.partialMedia = weblens.New(contentId)
...
func createZipFromPaths(t *task) {
	...
	paths := internal.MapToKeys(filesInfoMap)
	slices.Sort(paths)
	...
	owner := t.taskPool.workerPool.userService.Get(zipMeta.username)
	takeoutRoot := t.taskPool.workerPool.fileTree.Get("TAKEOUT")

	zipFile, err := t.taskPool.workerPool.fileTree.Touch(takeoutRoot, zipName, false, owner)
...
func moveFile(t *task) {
	...
	file := t.taskPool.workerPool.fileTree.Get(moveMeta.fileId)
	if file == nil {
		t.ErrorAndExit(errors.New("could not find existing file"))
	}

	acc := dataStore.NewAccessMeta(t.taskPool.workerPool.userService.GetRootUser())
	moveEvent := fileTree.NewFileEvent()

	destinationFolder := t.taskPool.workerPool.fileTree.Get(moveMeta.destinationFolderId)
	if destinationFolder == destinationFolder.Owner().GetTrashFolder() {
		err := weblens.MoveFileToTrash(file, acc, moveEvent, t.caster)
		if err != nil {
			t.ErrorAndExit(err, "Failed while assuming move file was to trash")
		}
		return
	} else if weblens.IsFileInTrash(file) {
		err := weblens.ReturnFileFromTrash(file, moveEvent, t.caster)
		if err != nil {
			t.ErrorAndExit(err, "Failed while assuming move file was out of trash")
		}
		return
	}
	err := t.taskPool.workerPool.fileTree.Move(
		file, destinationFolder, moveMeta.newFilename, false, moveEvent, t.caster.(websocket.BufferedBroadcasterAgent),
	)
	if err != nil {
		t.ErrorAndExit(err)
	}
...
func handleFileUploads(t *task) {
	...
	rootFile := t.taskPool.workerPool.fileTree.Get(meta.rootFolderId)
	if rootFile == nil {
		t.ErrorAndExit(
	...
	var bufCaster websocket.BufferedBroadcasterAgent
	switch t.caster.(type) {
	case websocket.BufferedBroadcasterAgent:
		bufCaster = t.caster.(websocket.BufferedBroadcasterAgent)
	default:
	...
	defer func() {
		if t.signal.Load() == 0 {
			err = t.taskPool.workerPool.fileTree.GetJournal().LogEvent(fileEvent)
			if err != nil {
				wlog.ShowErr(err)
			}
		}
	...
	for {
		...
		case chunk := <-meta.chunkStream:
			...
			if fileMap[chunk.FileId].bytesWritten >= fileMap[chunk.FileId].fileSizeTotal {
				...
				_, err := weblens.GenerateContentId(fileMap[chunk.FileId].file)
				if err != nil {
					t.ErrorAndExit(
				...
				err = t.taskPool.workerPool.fileTree.AttachFile(fileMap[chunk.FileId].file, bufCaster)
				if err != nil {
					wlog.ShowErr(err)
				}
...
func (t *task) NewFileInStream(file *fileTree.WeblensFile, fileSize int64) error {
	switch t.taskType {
	case WriteFileTask:
	default:
...
func hashFile(t *task) {
	...
	if meta.file.IsDir() {
		t.ErrorAndExit(
			error2.NewWeblensError("cannot hash directory"),
	...
	var contentId weblens.ContentId
	...
	contentId = weblens.ContentId(base64.URLEncoding.EncodeToString(newHash.Sum(nil)))[:20]
