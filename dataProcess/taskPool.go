...
func (tp *taskPool) newTask(
	taskType types.TaskType, taskMeta types.TaskMetadata, caster websocket.BroadcasterAgent,
) types.Task {

	var taskId types.TaskId
	if taskMeta == nil {
		taskId = types.TaskId(internal.GlobbyHash(8, time.Now().String()))
	} else {
		taskId = types.TaskId(internal.GlobbyHash(8, taskMeta.MetaString(), taskType))
	}
	...
	newTask := &task{
		...
		sw: internal.NewStopwatch(fmt.Sprintf("%s Task [%s]", taskType, taskId.String())),
		caster: caster,
	}
...
func (tp *taskPool) ScanDirectory(directory *fileTree.WeblensFile, caster websocket.BroadcasterAgent) types.Task {
	...
	t := tp.newTask(ScanDirectoryTask, scanMeta, caster)
	err := tp.QueueTask(t)
	if err != nil {
		wlog.ErrTrace(err)
		return nil
...
func (tp *taskPool) ScanFile(file *fileTree.WeblensFile, caster websocket.BroadcasterAgent) types.Task {
	scanMeta := scanMetadata{file: file}
	t := tp.newTask(ScanFileTask, scanMeta, caster)
	err := tp.QueueTask(t)
	if err != nil {
		wlog.ErrTrace(err)
		return nil
...
func (tp *taskPool) WriteToFile(
	...
	t := tp.newTask(WriteFileTask, writeMeta, caster)
...
func (tp *taskPool) MoveFile(
	...
	t := tp.newTask(MoveFileTask, moveMeta, caster)
	err := tp.QueueTask(t)
	if err != nil {
		wlog.ErrTrace(err)
		return nil
...
func (tp *taskPool) CreateZip(
	files []*fileTree.WeblensFile, username types.Username, shareId types.ShareId,
	casters websocket.BroadcasterAgent,
) types.Task {
	meta := zipMetadata{files: files, username: username, shareId: shareId}
	t := tp.newTask(CreateZipTask, meta, casters)
	if c, _ := t.Status(); !c {
		err := tp.QueueTask(t)
		if err != nil {
			wlog.ErrTrace(err)
...
func (tp *taskPool) GatherFsStats(rootDir *fileTree.WeblensFile, caster websocket.BroadcasterAgent) types.Task {
	t := tp.newTask(GatherFsStatsTask, fsStatMeta{rootDir: rootDir}, caster)
	err := tp.QueueTask(t)
	if err != nil {
		wlog.ErrTrace(err)
		return nil
...
func (tp *taskPool) Backup(remoteId types.InstanceId, caster websocket.BroadcasterAgent) types.Task {
	t := tp.newTask(BackupTask, backupMeta{remoteId: remoteId}, caster)
	err := tp.QueueTask(t)
	if err != nil {
		wlog.ErrTrace(err)
		return nil
...
func (tp *taskPool) CopyFileFromCore(
	...
	t := tp.newTask(CopyFileFromCore, backupCoreFileMeta{file: file, core: core}, caster)
	err := tp.QueueTask(t)
	if err != nil {
		wlog.ErrTrace(err)
		return nil
...
func (tp *taskPool) HashFile(f *fileTree.WeblensFile, caster websocket.BroadcasterAgent) types.Task {
	t := tp.newTask(HashFile, hashFileMeta{file: f}, caster)
	err := tp.QueueTask(t)
	if err != nil {
		wlog.ErrTrace(err)
		return nil
	...
	if !tp.treatAsGlobal {
		...
		if uncompletedTasks == 0 && tp.allQueuedFlag.Load() {
			if tp.waiterCount.Load() != 0 {
				wlog.Debug.Println("Pool complete, waking sleepers!")
			...
			if tp.cleanupFn != nil {
				func() {
					defer func() {
						if r := recover(); r != nil {
							wlog.ShowErr(error2.NewWeblensError(fmt.Sprint(r)), "Failed to execute taskPool cleanup")
						}
					}()
					tp.cleanupFn()
				}()
...
func (tp *taskPool) NotifyTaskComplete(t types.Task, c websocket.BroadcasterAgent, note ...any) {
	realT := t.(*task)
...
func (tp *taskPool) Wait(supplementWorker bool) {
	...
	wlog.Debug.Printf("Parking on worker pool from %s:%d\n", file, line)
	...
	wlog.Debug.Printf("Woke up, returning to %s:%d\n", file, line)
...
func (tp *taskPool) Errors() []types.Task {
	...
	return internal.SliceConvert[types.Task](erroredTasks)
}
...
func (tp *taskPool) QueueTask(Task types.Task) (err error) {
	t := Task.(*task)
	if tp.workerPool.exitFlag.Load() == 1 {
		wlog.Warning.Println("Not queuing task while worker pool is going down")
		return
	}
	...
	if t.err != nil {
		...
		wlog.Warning.Println("Not re-queueing task that has error set, please restart weblens to try again")
		return
	}
	...
	if tp.allQueuedFlag.Load() {
		...
		return error2.WErrMsg("attempting to add task to closed task queue")
	}
...
func (tp *taskPool) SignalAllQueued() {
	if tp.treatAsGlobal {
		wlog.Error.Println("Attempt to signal all queued for global queue")
	}
