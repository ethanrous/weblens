...
func doBackup(t *task) {
	...
	localRole := InstanceService.GetLocal().ServerRole()
	...
	if localRole == types.Initialization {
		...
		t.ErrorAndExit(error2.WErrMsg("cannot run backup task on a core server"))
	}

	var proxyService types.ProxyStore
	var ok bool
	if proxyService, ok = types.SERV.StoreService.(types.ProxyStore); !ok {
		t.ErrorAndExit(error2.WErrMsg("cannot run backup task without proxy service initialized"))
	}
	localStore := proxyService.GetLocalStore()

	coreClient := types.SERV.ClientManager.GetClientByInstanceId(meta.remoteId)
	if coreClient == nil {
		t.ErrorAndExit(error2.WErrMsg("Core websocket not connected"))
	}
	...
	if len(pool.Errors()) != 0 {
		t.ErrorAndExit(error2.WErrMsg(fmt.Sprintf("%d backup file copies have failed", len(pool.Errors()))))
	}
...
func copyFileFromCore(t *task) {
	...
	var ok bool
	if proxyService, ok = types.SERV.StoreService.(types.ProxyStore); !ok {
		t.ErrorAndExit(error2.WErrMsg("cannot run copy core file task without proxy service initialized"))
	}
