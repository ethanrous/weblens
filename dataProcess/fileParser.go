...
func processMediaFile(t *task) {
	...
	file.AddTask(t)
	defer func(file *fileTree.WeblensFile, id types.TaskId) {
...
func scanDirectory(t *task) {
	...
	if types.SERV.MediaRepo == nil {
		t.ErrorAndExit(error2.NewWeblensError("cannot scan directory without valid initilized media repo"))
	}
	...
	scanDir.AddTask(t)
	defer func(scanDir *fileTree.WeblensFile, id types.TaskId) {
