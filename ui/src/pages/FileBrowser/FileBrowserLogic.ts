...
import { FileApi, SubToTask, downloadSingleFile } from '@weblens/api/FileBrowserApi'
import Upload, { FileUploadMetadata, UPLOAD_CHUNK_SIZE } from '@weblens/api/Upload'
...
async function addDir(
    ...
    if (fsEntry.isDirectory) {
        ...
        for (const entry of allEntries) {
            allResults.push(...(await addDir(entry, uploadId, folderId, topFolderKey, rootFolderId, isPublic, shareId)))
        }
        return allResults
    } else {
...
export async function HandleDrop(
    ...
    if (entries) {
        ...
        for (const entry of entries) {
            topLevels.push(
                addDir(entry, uploadId, rootFolderId, null, rootFolderId, isPublic, shareId)
                    .then((newFiles) => {
                        files.push(...newFiles)
                    })
                    .catch((r) => {
...
export async function HandleUploadButton(files: File[], parentFolderId: string, isPublic: boolean, shareId: string) {
    ...
    if (uploads.length !== 0) {
        Upload(uploads, isPublic, shareId, res.data.uploadId).catch(ErrorHandler)
    }
}
...
export function calculateShareId(files: WeblensFile[]): string {
    ...
    if (state.folderInfo.id === 'shared') {
        if (files.length !== 1) {
            throw new Error('Cannot download multiple files from the shared folder')
        }
...
export async function downloadSelected(
    ...
    if (files.length === 1 && !files[0].IsFolder()) {
        return downloadSingleFile(files[0].Id(), files[0].GetFilename(), shareId, format)
    }

    return FileApi.createTakeout({ fileIds: files.map((f) => f.Id()) }, shareId).then((res) => {
        if (res.status === 200) {
            if (!res.data.takeoutId || !res.data.filename) {
                throw new Error('Takeout ID or filename is missing in the takeout response')
            }

            downloadSingleFile(res.data.takeoutId, res.data.filename, shareId, 'zip').catch(ErrorHandler)
        } else if (res.status === 202) {
            if (!res.data.taskId) {
                throw new Error('Task ID is missing in the takeout response')
            }
...
export const useKeyDownFileBrowser = () => {
    ...
    const setPresentationTarget = useFileBrowserStore((state) => state.setPresentationTarget)
    ...
    const setHoldingShift = useFileBrowserStore((state) => state.setHoldingShift)
    const setPresentation = useFileBrowserStore((state) => state.setPresentationTarget)
        ...
        const onKeyDown = (event: KeyboardEvent) => {
            ...
            if (!blockFocus) {
                if ((event.metaKey || event.ctrlKey) && event.key === 'a') {
                    ...
                    const newTarget = filesLists.get(folderInfo.Id())[presentingTarget.GetIndex() + direction]
                    if (!newTarget) {
                        return
                    }
                    setPresentationTarget(newTarget.Id())

                    const onDeck = filesLists.get(folderInfo.Id())[presentingTarget.GetIndex() + direction * 2]
                    if (onDeck) {
                        const m = mediaMap.get(onDeck.GetContentId())
                        if (m && !m.HasQualityLoaded(PhotoQuality.HighRes)) {
                            m.LoadBytes(PhotoQuality.HighRes).catch(ErrorHandler)
                        }
                    }
                } else if (
                    event.key === 'Escape' &&
    ...
    }, [blockFocus, searchContent, presentingId, lastSelected, isSearching, menuMode])
}
                        ...
                        for (const item of items) {
                            for (const mime of item.types) {
                                if (mime.startsWith('image/')) {
                                    ...
                                    const img: ArrayBuffer = await (await item.getType(mime)).arrayBuffer()
                                    setPaste(img)
