...
import { Box, Divider, Input, Space, Text, TextInput, Tooltip, TooltipFloating } from "@mantine/core"
...
const SingleAlbum = memo(({ album, albumCoverId, setMediaCallback, loading, PartialApiCall }: { album: AlbumData, albumCoverId: string, setMediaCallback: (mediaId: string, quality: "thumbnail" | "fullres", data: ArrayBuffer) => void, loading: boolean, PartialApiCall: (albumId: string) => void }) => {
    const { userInfo } = useContext(userContext)
    return (
        <Box className="album-preview-row" style={{ pointerEvents: loading ? 'none' : 'all' }} onClick={() => { PartialApiCall(album.Id) }}>
            {loading && (
                <Box className="album-preview-loading" onClick={e => e.stopPropagation()} />
            )}
            <MediaImage media={album.CoverMedia} setMediaCallback={setMediaCallback} quality='thumbnail' expectFailure={album.Cover === ""} containerStyle={{ borderRadius: '5px', overflow: 'hidden', width: '65px', height: '65px' }} />
            <RowBox style={{ width: '235px', justifyContent: 'space-evenly', flexGrow: 0 }}>
                <ColumnBox style={{ height: 'max-content', width: '50%', alignItems: 'flex-start', flexGrow: 1 }}>
                    <Box style={{ display: 'flex', flexGrow: 0, width: 'max-content', maxWidth: '100%', alignItems: 'center', paddingBottom: '10px' }}>
                        <Tooltip openDelay={200} label={album.Name}>
                            <Text c='white' size="16px" fw={550} truncate='end' styles={{ root: { width: '100%' } }}>{album.Name}</Text>
                        </Tooltip>
                        {album.Owner !== userInfo.username && (
                            <Tooltip label={`Shared by ${album.Owner}`}>
                                <IconUsersGroup color="white" size={'20px'} style={{ marginLeft: 10 }} />
                            </Tooltip>
                        )}
                    </Box>
                    <RowBox>
                        <RowBox>
                            <IconPhoto size={'15px'} />
                            <Text size='15px' style={{ paddingLeft: 5 }}>{album.Medias.length}</Text>
                        </RowBox>

                    </RowBox>
                </ColumnBox>
                <RowBox style={{ position: 'absolute', width: 'max-content', alignItems: 'flex-end', justifyContent: 'flex-end', padding: 4, right: 0 }}>
    ...
    console.log("Checkin", prev.albumCoverId, prev.albumCoverId)
    if (prev.loading !== next.loading) {
        return false
    } else if (prev.albumCoverId !== next.albumCoverId) {
        return false
    }
...
export const AlbumScoller = memo(({ getSelected, authHeader }: {
    ...
    return (
        <ColumnBox style={{ maxHeight: 660, width: 320 }}>
            ...
            <VariableSizeList
                ...
                height={75 * filteredAlbums?.length < 600 ? 75 * filteredAlbums.length : 600}
                width={'100%'}
                itemKey={(index: number, data: AlbumData[]) => data[index]?.Id}
            >
                {({ data, index, style }) => AlbumRowWrap(data, index, style, loadingAlbums, setMediaCallback, addMediaApiCall)}
            </VariableSizeList>
        </ColumnBox>
    )
}, (prev, next) => {
...
const AlbumRowWrap = (data: AlbumData[], index, style, loadingAlbums, setMediaCallback, create) => {
    return (
        <Box style={style}>
            <SingleAlbum setMediaCallback={setMediaCallback} album={data[index]} albumCoverId={data[index].Cover} loading={loadingAlbums.includes(data[index].Id)} PartialApiCall={create} />
        </Box>
    )
