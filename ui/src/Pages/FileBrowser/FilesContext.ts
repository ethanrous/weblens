...
import { FbMenuModeT, FileContextT, GlobalContextType, SelectedState } from '../../Files/File';
...
export function GetFilesContext(
    ...
    const context: GlobalContextType = {
        ...
        setHovering: (i: string) => dispatch({ type: 'set_hovering', hovering: i }),
        setDragging: (d: DraggingStateT) => dispatch({ type: 'set_dragging', dragging: d }),
        doSelectMany: (): string[] => {
            if (lastSelectedIndex === -1) {
                return [fb.hovering];
            }

            return hoveringIndex > lastSelectedIndex
                ? files.slice(lastSelectedIndex, hoveringIndex + 1).map(v => v.file.Id())
                : files.slice(hoveringIndex, lastSelectedIndex + 1).map(v => v.file.Id());
        },
        moveSelected: (entryId: string) => {
            if (fb.dirMap.get(entryId).IsFolder()) {
                MoveSelected(fb.selected, entryId, authHeader).then(() => dispatch({ type: 'clear_selected' }));
            }
        },
        blockFocus: (b: boolean) => dispatch({ type: 'set_block_focus', block: b }),
        rename: (itemId: string, newName: string) => handleRename(itemId, newName, dispatch, authHeader),
        setMenuOpen: (m: FbMenuModeT) => dispatch({ type: 'set_menu_open', menuMode: m }),
...
export function GetItemsList(
    ...
    let filesList = Array.from(fb.dirMap.values()).filter(val => {
        ...
        return val.GetFilename().toLowerCase().includes(debouncedSearch.toLowerCase()) && !val.IsTrash();
    });
    ...
    const ctxS = filesList.map((v, i) => {
        ...
        const ctx: FileContextT = {
            ...
            selected:
                ...
                (fb.holdingShift && (i - hoveringIndex) * (i - lastSelectedIndex) < 1 && SelectedState.InRange) |
