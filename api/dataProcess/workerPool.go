...
type WorkerPool struct {
	...
	remainingTasks chan workWrapper
...
func NewWorkerPool(workerCount int) (WorkerPool) {
	...
	newWp := WorkerPool{
		...
		remainingTasks: make(chan workWrapper, workerCount * 1000),
		busyCount: &busyCount,
...
func (wp *WorkerPool) AddTask(f func()) {
	wp.totalTasks.Add(1)
	wp.remainingTasks <- workWrapper{work: f, flag: 0}
}
...
func (wp *WorkerPool) Close() {
	for wp.currentWorkers.Load() > 0 {
		wp.remainingTasks <- workWrapper{flag: 1}
	}
}
