...
func formatRespondFolderInfo(dir *fileTree.WeblensFile, ctx *gin.Context) {
	u := getUserFromCtx(ctx)
	FileService.FormatFileInfo(dir, u, nil)
	...
	var filteredDirInfo []weblens.FileInfo
	if dir.IsDir() {
		if acc.GetTime().Unix() > 0 {
			...
			FileService.FormatFileInfo(dir, acc.User(), nil)
			filteredDirInfo = dir.GetChildrenInfo(acc)
		}
	} else {
		filteredDirInfo = []weblens.FileInfo{}
	}

	var parentsInfo []weblens.FileInfo
	parent := dir.GetParent()
	for acc.CanAccessFile(parent) && parent != dir && (parent.Owner() != UserService.Get("WEBLENS")) {
	...
	if slices.ContainsFunc(
		dir.GetChildren(), func(f *fileTree.WeblensFile) bool {
...
func getFolder(ctx *gin.Context) {
	...
	if sh != nil {
		err = acc.AddShare(sh)
		if errors.Is(err, weblens.ErrUserNotAuthorized) {
...
func getSharedFiles(ctx *gin.Context) {
	...
	filesInfos := internal.FilterMap(
		shares, func(sh weblens.Share) (weblens.FileInfo, bool) {
			err = acc.AddShare(sh)
...
func getDirectoryContent(ctx *gin.Context) {
	...
	children := internal.Map(
		f.GetChildren(), func(c *fileTree.WeblensFile) types.FileStat {
			size, err := c.Size()
			if err != nil {
				wlog.ShowErr(err)
