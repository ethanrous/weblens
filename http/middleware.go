...
func parseAuthHeader(authHeaderParts []string) (string, string, error) {
	...
	if len(authList) < 2 {
		...
		return "", "", error2.ErrBadAuthScheme
...
func validateBasicAuth(cred string) (types.User, error) {
	...
	if len(userAndToken) != 2 {
		return nil, error2.ErrBasicAuthFormat
	}

	u := UserService.Get(types.Username(userAndToken[0]))
	if u == nil {
...
func WebsocketAuth(c *gin.Context, authHeader []string) (types.User, types.Instance, error) {
	...
	if scheme == "" {
		...
		if key.RemoteUsing == "" {
			return nil, nil, error2.WErrMsg("Bad bearer token in websocket auth")
		}
		instance = InstanceService.Get(key.RemoteUsing)
		if instance == nil {
			return nil, nil, error2.WErrMsg("No remote using key in websocket auth")
		}
	} else {
		return nil, nil, error2.ErrBadAuthScheme
	}
...
func WeblensLogger(c *gin.Context) {
	...
	if !internal.IsDevMode() && status < 400 {
		return
	}
