...
var FileService *weblens.FileServiceImpl
var AccessService weblens.AccessService
var UserService weblens.UserService
var ShareService weblens.ShareService
var InstanceService weblens.InstanceService
var AlbumService weblens.AlbumService
...
func DoRoutes() {
	...
	for {
		...
		if !InstanceService.IsLocalLoaded() {
			...
			if local.ServerRole() == weblens.InitServer {
				...
				if local.ServerRole() == weblens.CoreServer {
					addCoreRoutes(core)
				} else if local.ServerRole() == weblens.BackupServer {
